...
# Tách dòng cho tất cả trang
all_line_boxes = []
for i, (p, boxes) in enumerate(zip(pages, all_boxes)):
    img_w = p["img"].shape[1]
    line_boxes = []
    for box in boxes:
        line_boxes.extend(split_box_to_lines(box, img_w))
    all_line_boxes.append(line_boxes)

print(f"  → Trang 1: {len(all_boxes[0])} blocks → {len(all_line_boxes[0])} dòng")
print(f"  → Trang 2: {len(all_boxes[1])} blocks → {len(all_line_boxes[1])} dòng\n")
